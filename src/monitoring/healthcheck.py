...
# Logging configuration for Docker logs
logging.basicConfig(
    level=logging.INFO,
...
def check_kafka():
    """Check Kafka availability by attempting connection"""
    try:
...
def check_postgres():
    ...
    try:
        # Using credentials from settings / environment
        conn = psycopg2.connect(
            host="postgres",  # Service name in docker-compose
            port=5432,
            database="twitter_sentiment",
            user="admin",
            # In Docker, password is usually provided via ENV
